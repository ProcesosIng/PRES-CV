...
import { listarForecastComercial, listarRegistrosParaCosteo, guardarRegistrosLote, obtenerProductosOdoo, obtenerCuentasOdoo, obtenerCostoEmbalajePorProducto } from '../../data/store';
import '../../index.css';

const ANIO_ACTUAL = new Date().getFullYear();
const ANIOS_DISPONIBLES = Array.from({ length: 5 }, (_, i) => (ANIO_ACTUAL - 1 + i).toString());
const MODULOS_EXCLUIDOS_DESTINO = ['Costeo de Crisoles', 'Costeo de Fundente', 'Costeo de Copelas', 'Costeo de Embalajes', 'Forecast de Ventas'];
...
// Copelas usa Primer Proceso, Segundo Proceso y CIF (igual que Crisoles).
...
// Cuenta con prefijo 93 (Producción Copelas): conserva los últimos 7 dígitos y antepone 93.
// (Así '946261000' sale como '936261000' y no con 11 dígitos.)
const formatearCuentaConPrefijo93 = (cuentaOriginal) => {
  if (!cuentaOriginal) return '93';
  ...
  return `93${base} ${resto}`.trim();
};

export default function CosteoCopelasForm({ registro, onGuardar, onCancelar, modo, idVersion, area }) {
  const isSoloLectura = modo === 'ver';

  const forecastCopelas = useMemo(() => {
  ...
  // Copelas NO tiene lista de materiales (BOM) creada en Odoo: al marcar un
  // producto solo se agrega una fila vacía para ingresar sus materiales a mano.
  ...
  const productosAgrupados = useMemo(() => {
    ...
    forecastCopelas.forEach(reg => {
      ...
      const esApto = String(dc.unidad_negocio || '').toLowerCase().trim() === 'copelas';
  ...
  useEffect(() => {
    if (productosSeleccionados.length > 0 && !registro) {
      ...
      productosSeleccionados.forEach(nombreProd => {
        const registrosDelProd = forecastCopelas.filter(r => {
  ...
  }, [anioSel, forecastCopelas, productosSeleccionados]);
  ...
  }, [productosBase, forecastCopelas, anioSel, configProductos]);
    ...
    productosSeleccionados.forEach((nombreProd, pIdx) => {
      ...
      const derivar = (detalleLista, tag, moduloPorDefecto) => {
        detalleLista.forEach((d, idx) => {
          ...
          const cuentaConPrefijo = formatearCuentaConPrefijo93(cuentaSegunModulo(moduloDestino));
        ...
        lista.forEach((mod, i) => {
          ...
          if (monto > 0) {
            desgloseProdContable.push({
              id: `mod-${etiqueta}-${i}`,
              cuenta: formatearCuentaConPrefijo93(`946261000 - ${mod.moduloNombre || etiqueta} (${etiqueta})`),
      ...
      const regForecast = forecastCopelas.find(r => {
        const dc = r.detalle_columnas || r;
      ...
      const registroMaestroProd = {
        ...
        id_lote: idLoteBase,
        modulo: 'Costeo de Copelas',
        categoria: 'Costeo de Copelas',
        area,
        idVersion,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%' }}>
      ...
      <fieldset disabled={isSoloLectura} style={{ border: 'none', padding: 0, margin: 0, flex: 1, display: 'flex', flexDirection: 'column', minHeight: '0' }}>
        <div className="offcanvas-body" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto', padding: '16px' }}>
          ...
          <div className="form-section" style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', background: 'white', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1' }}>
              {productosAgrupados.length > 0 ? (
                ...
                <span style={{ fontSize: '12px', color: '#64748b', fontStyle: 'italic' }}>No se detectaron Productos Terminados o Copelas en el Forecast para este año.</span>
              )}
            </div>
          </div>
          ...
          {/* 2. MATERIALES (PROCESO 1) - AGRUPADO POR PRODUCTO, CARGA MANUAL (no hay lista de materiales en Odoo) */}
          <div className="form-section" style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            ...
            {materiales.length === 0 && <div style={{ padding: '12px', textAlign: 'center', fontSize: '12px', color: '#64748b' }}>Selecciona un producto arriba y agrega sus materiales manualmente.</div>}
                    ...
                    {itemsDelGrupo.map((item) => {
                      ...
                      return (
                        <div key={item.id} style={{ display: 'grid', gridTemplateColumns: 'minmax(160px, 2fr) 50px 80px 65px 65px 1.5fr 95px auto', gap: '6px', alignItems: 'center', marginBottom: '6px', minWidth: '750px' }}>
                          ...
                          <input type="text" value={item.cuenta || ''} readOnly placeholder={`(fija: ${formatearCuentaConPrefijo93('6121000')})`} style={{ padding: '6px', border: '1px solid #e2e8f0', borderRadius: '4px', fontSize: '10px', background: '#f8fafc', color: '#94a3b8' }} title="La cuenta de materias primas e insumos es fija: 6121000 con el prefijo del área" />
          ...
          <div className="form-section">
            ...
            {renderAsignacionProceso(
              ...
              'Se divide entre TODA la producción de copelas, proporcional al volumen de cada producto.'
            )}
            {renderAsignacionProceso(
          ...
          <div className="form-section">
            ...
            {renderAsignacionProceso(
              ...
              'Se reparte entre toda la producción de copelas, proporcional al volumen de cada producto.'
            )}
          </div>
          ...
          return (
            <div className="reporte-impresion-solo">
              <div className="reporte-header">
                <div className="reporte-titulo">REPORTE DE TRAZABILIDAD - COSTEO DE COPELAS</div>
